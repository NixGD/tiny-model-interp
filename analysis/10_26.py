...
next_token_letter_mask = get_mask_y_token(y, list(string.ascii_uppercase) + list(string.ascii_lowercase), tokenizer)
last_token_t_mask = ending_mask(x, "t", tokenizer)


masks = {}
masks["subdist"] = next_token_letter_mask & last_token_t_mask
masks["remainder"] = masks["subdist"]

for ending in [" wit", " t", " int", " st"]:
    masks[f"...{ending}"] = masks["subdist"] & ending_mask(x, ending, tokenizer)
    masks["remainder"] = masks["remainder"] & ~masks[f"...{ending}"]


for mask_name, mask in masks.items():
    print(f"{mask_name}: {mask.sum()}/{len(next_token_letter_mask)}")
